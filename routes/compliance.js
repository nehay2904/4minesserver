...
// PATCH /api/compliances/:id/upload-license
// Appends licence copies / renewal proofs to a License compliance without changing its status.
router.patch('/:id/upload-license', protect, upload.array('proofs', 20), async (req, res) => {
  try {
    const c = await Compliance.findById(req.params.id);
    if (!c) return res.status(404).json({ message: 'Compliance not found' });
    if (c.subCategory !== 'License')
      return res.status(400).json({ message: 'This route is only for License compliances' });
    const assignedIds = c.assignedTo.map((u) => String(u._id || u));
    const isOwner = assignedIds.includes(String(req.user._id));
    if (!isOwner && req.user.role !== 'admin')
      return res.status(403).json({ message: 'Access denied' });
